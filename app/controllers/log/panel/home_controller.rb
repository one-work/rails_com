module Log
  class Panel::HomeController < Panel::BaseController
    ...
    def index
      ...
      request_min = @request_dailies.minimum(:total)
      @request_dailies = @request_dailies.where(total: request_min..).limit(10)

      @query_dailies = QueryDaily.where(date: yesterday, total: 10..).order(duration_avg: :desc)
      query_min = @query_dailies.minimum(:total)
