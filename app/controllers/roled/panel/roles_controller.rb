module Roled
  class Panel::RolesController < Panel::BaseController
    ...
    def show
      ...
      @meta_controllers = Meta::Controller.includes(:meta_actions).default_where(q_params)
      @meta_businesses = Meta::Business.all
    end

    def namespaces
      @meta_business = Meta::Business.find_by identifier: params[:business_identifier]
    end

    def controllers
      @meta_namespace = Meta::Namespace.find_by identifier: params[:namespace_identifier]
      @meta_controllers = Meta::Controller.includes(:meta_actions).where(params.permit(:business_identifier, :namespace_identifier))
    end

    def actions
      @meta_controller = Meta::Controller.find params[:meta_controller_id]
    ...
    def namespace_on
      ...
      @meta_controllers = Meta::Controller.where(params.permit(:business_identifier, :namespace_identifier))
      @meta_business = Meta::Business.find_by identifier: params[:business_identifier]
    ...
    def action_on
      @meta_action = Meta::Action.find params[:meta_action_id]
